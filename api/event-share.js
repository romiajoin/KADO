...
//
// 是 api/share.js（機台分享）的活動版對照組：機台走永久ID（permId）比對，
// 活動目前沒有永久ID機制，直接用 A 欄流水號（跟 shareEvent() 產生的 ?event=<id> 一致）。
//
...
const TITLE = 'KADO！抽卡機在哪｜活動行事曆';
const DESCRIPTION = '動漫快閃店、聯名展覽、簽名會、特典活動⋯⋯全台 IP 活動資訊持續更新中！';
...
// cols[11] 更多資訊連結, cols[12] 營業時間, cols[13] 分享圖（N欄，新增，選填）
...
// 依 id（A 欄流水號）到活動分頁 CSV 裡找對應列。
// status:
//   'matched'     → 找到對應列，imageUrl 為該活動指定的分享圖（沒填就用預設圖）
//   'not-found'   → 表格抓取成功，但找不到任何 id 相符的列（已下架或根本不存在的 id）
...
async function getShareInfo(id) {
  ...
  try {
    ...
    const match = parsed.find((cols) => cols[0] === id);
    if (match) {
...
module.exports = async function handler(req, res) {
  ...
  // 只有「確定找到這個 id」或「抓表失敗、不確定是否存在」時才帶 id 導去活動頁
