...
import { initTopBarScroll, resetTopBarScrollState } from '../pages/machines/scroll.js';
    ...
    // =============================================
    // 📥 載入 Google Sheet
    // 「最後更新」時間的轉換／比較邏輯（to24Hour／parseUpdateDate／buildLastUpdatedText）
    // 已搬到 utils.js 共用，events-header.js 也是 import 同一份，不再各自維護一份。
    // =============================================
    export async function loadFromSheet(opts) {
      ...
      try {
        ...
        const lastUpdatedText = buildLastUpdatedText(machineLastUpdatedRaw, eventsLastUpdatedRaw);
    ...
    document.addEventListener('click', function (e) {
      ...
      // 作品標籤點擊快速篩選（grid modal／地圖詳情面板共用同一顆按鈕）：改用 data-* 屬性 +
      // 這裡的事件委派，不再靠 inline onclick + window 掛載，也順便拿掉手動跳脫單引號那段
      // 風險（作品名稱含特殊字元時 inline onclick 字串可能被拼斷），見 CLAUDE.md「作品標籤點擊快速篩選」。
      const charBtn = e.target.closest('.popup-character-link[data-character]');
      if (charBtn) {
        filterByCharacter(charBtn.dataset.character, charBtn.dataset.machineId, charBtn.dataset.source);
        return;
      }

      // 「在 Google Maps 查看」點擊追蹤：連結本身用原生 <a href target="_blank"> 導頁，
      // 這裡只負責送 GA 事件，不攔截預設行為（不 preventDefault，讓連結照常開新分頁）
      const gmapsLink = e.target.closest('[data-gmaps-track]');
      if (gmapsLink) {
    ...
    export function openGridModal(loc, imgs, googleMapsUrl, source = 'grid_modal') {
          ...
          <div class="popup-img-wrap">
            <div class="carousel" id="${mid}" data-index="0" data-imgs='${JSON.stringify(imgs)}'>
              ...
              <div class="carousel-controls">
                <button class="carousel-btn" data-carousel-action="prev" data-carousel-id="${mid}" aria-label="上一張圖片">${CAROUSEL_CHEVRON_ICON_SVG}</button>
                <span class="carousel-counter">${1} / ${imgs.length}</span>
                <button class="carousel-btn carousel-btn-next" data-carousel-action="next" data-carousel-id="${mid}" aria-label="下一張圖片">${CAROUSEL_CHEVRON_ICON_SVG}</button>
              </div>
            </div>
          </div>`;
      }

      // 只算一次，避免同一個判斷式呼叫兩次（grid.js／map.js 都已經是這樣寫，這裡補齊）
      ...
      document.getElementById('gridModalContent').innerHTML = `
        ...
        <div class="modal-info-section">
          ...
          ${loc.character ? `<div class="popup-addr">作品：<button type="button" class="popup-character-link" data-character="${loc.character}" data-machine-id="${loc.id}" data-source="${source}">${loc.character}</button></div>` : ''}
    ...
    export function trackGmapsClick(id, source) {
    ...
    // 不要無預警跳去列表模式。
    export function filterByCharacter(character, machineId, source) {
    ...
    const lightbox = createLightbox('lightbox', 'lightboxImg'); // 開關邏輯共用，見 utils.js
