...
// 這裡的排序刻意跟 weekEventBars() 的 bars.sort() 用同一套比較邏輯（剩餘天數
// 優先，快結束的排最前面；同樣剩餘天數才比開始日早的、再比活動時間跨度長的），
// 讓 day-events-panel 清單的順序跟月曆橫幅由上到下看到的順序一致——同一天的
// 活動一定互相重疊（都涵蓋這一天），weekEventBars() 的貪婪排列在這種情況下
// 每個活動一定各自佔一列，列的順序就等於排序後的順序，所以這裡不用重算欄位
// 佔用，直接照同一套 sort 規則排就能對上月曆視覺上的順序。
function eventsForDate(date) {
  const today = new Date();
  return visibleGroups()
    .filter((g) => isWithin(date, g))
    .sort((a, b) => daysUntilEnd(a, today) - daysUntilEnd(b, today)
      || a.start - b.start
...
// 排序用：這個活動群組距離結束還剩幾天（以「日」為單位，跟 isWithin() 一樣只看
// 年/月/日，不管時分秒）。已經結束的回傳 Infinity，讓它們排到最後面——過去的
// 活動已經沒有「快沒了、要趕快去」的急迫性，不該搶到最前面的位置。
function daysUntilEnd(group, today) {
  const end = new Date(group.end.getFullYear(), group.end.getMonth(), group.end.getDate());
  const base = new Date(today.getFullYear(), today.getMonth(), today.getDate());
  const diff = Math.round((end - base) / 86400000);
  return diff < 0 ? Infinity : diff;
}

function weekEventBars(week, groups, today) {
  const visibleCols = [];
  ...
  // 排序：改成「剩餘天數」優先（快結束的排最前面，提醒使用者要趕快去），
  // 同樣剩餘天數才退回原本的橫向排列（colStart 早的優先、同開始日再比
  // colSpan 長的優先）。注意這個排序會影響下面貪婪打包演算法塞進第幾列的
  // 順序，所以跟「開始日早的、跨越天數長的」優先排列比起來，行動導向排序
  // 可能會讓同一週用到比較多列（打包效率變差），是刻意的取捨，見
  // CLAUDE.md／跟使用者的討論。
  bars.sort((a, b) => daysUntilEnd(a.group, today) - daysUntilEnd(b.group, today)
...
function renderWeekRow(week, today, weekKey, groups) {
  ...
  const bars = weekEventBars(week, groups, today);
  ...
  const toggleHtml = hasMore
    ? `<button type="button" class="events-week-toggle" data-week-key="${weekKey}">`
      + `${expanded ? '部分顯示' : '全部顯示'}`
      + `<svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor"><path d="M312-312h132q15.3 0 25.65 10.29Q480-291.42 480-276.21t-10.35 25.71Q459.3-240 444-240H276q-15.3 0-25.65-10.35Q240-260.7 240-276v-168q0-15.3 10.29-25.65Q260.58-480 275.79-480t25.71 10.35Q312-459.3 312-444v132Zm336-336H516q-15.3 0-25.65-10.29Q480-668.58 480-683.79t10.35-25.71Q500.7-720 516-720h168q15.3 0 25.65 10.35Q720-699.3 720-684v168q0 15.3-10.29 25.65Q699.42-480 684.21-480t-25.71-10.35Q648-500.7 648-516v-132Z"/></svg>`
      + `</button>`
    : '';
...
function onDayCellHoverEnter(cell) {
  ...
  // hover-col 的上邊框疊在 sticky 的 .events-week-daynums 之上時，z-index
  // 不保證贏過 sticky 元素自己的合成層（見 events.css .col-hover 註解），
  // 另外在日期數字格自己身上補畫上／左／右三邊，湊出完整外框。
...
// 圖示／顏色 class 已搬到 utils.js（MACHINE_TYPE_BADGE_ICON／machineTypeClass）共用，
// grid.js／main.js／map.js 也是 import 同一份，不再各自複製。
function machineTypeBadgesHtml(group) {
  const types = relatedMachineTypesForGroup(group, allMachines);
  if (!types.size) return '';
  return ['抽卡機', '相卡機']
    .filter((t) => types.has(t))
    .map((t) => `<span class="type-badge ${machineTypeClass(t)}">${MACHINE_TYPE_BADGE_ICON[t]} ${t}</span>`)
    .join('');
}

// day-events-panel（當日活動 drawer）卡片：沿用跟總覽·列表（eventListCardHtml()）
// 同一套 .loc-card-grid 視覺語言（type-badge／已結束灰階／作品·縣市 tag 都統一），
// 差別只在這裡不需要「詳情」按鈕——drawer 本來就是整張卡片可點擊展開詳情，按鈕在
// 400px 窄的側欄/bottom sheet 裡純屬多餘；共用的卡片內容交給 locCardTopHtml()。
function eventGroupCardHtml(group) {
  ...
  return `
    <div class="loc-card-grid${ended ? ' is-ended' : ''}${isSelected ? ' selected' : ''}" data-group-key="${group.key}">
      ${locCardTopHtml(group, { ended, badge })}
    </div>`;
}
...
// 把靜態 HTML 裡的兩顆快速篩選 pill（#eventsTimeFilterPill／#eventsMachineFilterPill）
// 搬進 #eventsFilterBar，插在篩選 pill（filterWidget 畫的 .filter-scroll）跟排序按鈕
// （sortWidget 動態 append 的 .sort-group）中間；因為 sortWidget.render() 是把
// .sort-group 直接 append 到 #eventsFilterBar 尾端，所以這裡要在 sortWidget.render()
// 之前呼叫，讓這兩顆先卡位（此時 .sort-group 還不存在，直接 appendChild 到尾端＝
// pills 後面），sortWidget 再把 .sort-group append 上去就會排在最後。
// filterWidget.render() 之後也要重新呼叫一次：它會清空 #eventsFilterBar 整個 innerHTML
// 重畫 pill，把之前搬進來的這兩顆一併清掉，需要再搬一次。
// 兩顆各自是 .filter-scroll 的獨立子元素（沒有额外的 .quick-filter-group wrapper），
// 這樣才能吃到 .filter-scroll 的 gap:12px，跟類型/作品/縣市 pill 間距一致。
function positionQuickFilterGroup() {
  const ongoingPill = document.getElementById('eventsTimeFilterPill');
  const machinePill = document.getElementById('eventsMachineFilterPill');
  if (!ongoingPill && !machinePill) return;
  const bar = document.getElementById('eventsFilterBar');
  if (!bar) return;
  // 一定要搬進 .filter-scroll 裡面，跟類型/作品/縣市 pill 同一個可橫向捲動
  // 的容器，手機版（<=991px）才能跟著一起左右滑動；放在 .filter-scroll
  ...
  const target = scrollEl || bar;
  if (ongoingPill) target.appendChild(ongoingPill);
  if (machinePill) target.appendChild(machinePill);
}
...
// v35.1：比照首頁（見 CLAUDE.md「排序系統」v35.1 條目）拿掉 end_date_desc、新增 start_date_asc。
...
// v?? 追加：.collage-card 補上 min-width: 330px（events.css）之後，原本純靠螢幕寬度分段的固定斷點
// （420/720/1100）會在欄數跟卡片最小寬度對不上時把卡片擠爆容器、造成橫向溢出/破版。改成量測
// #eventsCollageGrid 實際可用寬度，用「能塞下幾個 330px 欄」反推欄數（無 gap，欄與欄緊貼，見
// events.css 拼貼卡片圓角只在首尾欄外側加做法），永遠保證每欄至少有 330px，欄數上限維持原本的 4，
// 下限 1（螢幕再窄也至少顯示一欄，不會出現 0 欄）。
const COLLAGE_CARD_MIN_WIDTH = 330;
const COLLAGE_MAX_COLS = 4;
function collageColumnCount() {
  ...
  const cols = Math.floor(width / COLLAGE_CARD_MIN_WIDTH);
  return Math.max(1, Math.min(COLLAGE_MAX_COLS, cols));
}
...
// .loc-card-grid 共用內容（type-badge／多地點 pill／機台類型徽章／已結束或倒數 badge／
// 標題／期間／作品·縣市·場地 tag），eventListCardHtml()（總覽·列表，帶「詳情」按鈕）跟
// eventGroupCardHtml()（day-events-panel，無按鈕）共用同一份，只有外層要不要包 .card-actions
// 不一樣，由呼叫端各自決定，這裡只回傳 .card-top 這一塊。
function locCardTopHtml(group, { ended, badge }) {
  ...
  return `
    <div class="loc-card-grid${ended ? ' is-ended' : ''}" data-group-key="${group.key}">
      ...
      <div class="card-actions">
        <button class="btn-expand" type="button">詳情 ${BTN_EXPAND_ICON_SVG}</button>
      </div>
    </div>`;
}
...
// v35.1：比照首頁排序調整（見 CLAUDE.md「排序系統」v35.1 條目），拿掉 end_date_desc、
...
function openDayEventsPanel(key) {
  ...
  document.getElementById('dayEventsBody').querySelectorAll('.loc-card-grid').forEach((card) => {
    card.classList.add('is-clickable');
    card.addEventListener('click', () => {
      openGroupFromKey(card.dataset.groupKey, 'events_day_panel');

      document.getElementById('dayEventsBody').querySelectorAll('.loc-card-grid').forEach((c) => {
...
// 圖片放大 Lightbox：跟機台 modal（main.js openLightbox/closeLightbox）共用同一套 utils.js
// 開關邏輯（createLightbox），差異是這裡不走 inline onclick + window 掛載，用跟本檔其他
// overlay 一致的 addEventListener 綁定，詳見 CLAUDE.md「Lightbox」。
const eventLightbox = createLightbox('eventLightbox', 'eventLightboxImg');
function openEventLightbox(src) {
  eventLightbox.open(src);
  ...
  return `
    <div class="modal-info-section related-machines-section">
      ...
      <div class="related-machines-wrap">
        ...
        <button type="button" class="related-machines-nav-btn prev" data-nav="prev" aria-label="向左捲動相關機台" disabled>${CAROUSEL_CHEVRON_ICON_SVG}</button>
        <button type="button" class="related-machines-nav-btn next" data-nav="next" aria-label="向右捲動相關機台">${CAROUSEL_CHEVRON_ICON_SVG}</button>
      </div>
    </div>`;
}
...
function characterChipsHtml(loc) {
  ...
  const toggle = rest.length
    ? ` <button type="button" class="character-chips-toggle" id="eventDetailCharacterToggle">+${rest.length} 作品</button>` : '';
...
function locationSectionHtml(group, locIndex) {
  ...
  return `
    ...
    <div class="popup-actions">
      ...
      <button class="popup-share-btn" id="eventDetailShare">分享 ${SHARE_BTN_ICON_SVG}</button>
    </div>`;
}
...
function closeEventDetailModal(e) {
  ...
  openModalState = null;
  // day-events-panel 的卡片清單是 openDayEventsPanel() 渲染一次、之後只用點擊
  // handler 手動切換 .selected（見該函式，不整份重繪），renderAll() 只會重畫
  // 月曆/總覽本身，不會碰到 #dayEventsBody 這份既有的 DOM——關閉詳情 Modal 時
  // 這裡也要手動清掉 .selected，不然點過的那張卡片會一直帶著藍色外框，
  // 即使 state.selectedGroupKey 已經重設成 null 也不會反映到畫面上。
  document.getElementById('dayEventsBody')?.querySelectorAll('.loc-card-grid.selected')
