...
**⚠️ 機台狀態一鍵回報（「機台還在／不在了」）（本檔案這次更新時）前端尚未 commit／push（Worker 已部署，見下方「部署前置作業已完成」）**：痛點是「使用者發現機台不在了但懶得填回報表單」，做成 grid modal／地圖詳情面板（桌機側邊欄／mobile bottom sheet）共用的兩顆小按鈕，點下去立刻送出、不用填任何欄位。動到 `js/shared/utils.js`（新增 `machineReportButtonsHtml()`／`submitMachineReport()`／`getMachineReportState()`／`machineReportedText()` 等）、`js/core/main.js`（grid modal 插入按鈕、既有 document click 委派監聽器新增 `[data-machine-report]` 分支）、`js/ui/map.js`（地圖詳情面板插入按鈕，只有非 compact 版本才有，跟其他 `.popup-actions` 內容一致）、`css/style.css`（`.popup-report-group`／`.popup-report-btn`／`.popup-report-done`）、`scripts/worker.js`（跟現有 visitor-counter 共用同一支 worker，新增 `POST /report`，KV key 前綴 `report:`，成功後打 Discord webhook 通知）、`config/wrangler.toml`（補上 `DISCORD_WEBHOOK_URL` 這個密鑰要用 `wrangler secret put` 設定的說明，沒有把值寫進檔案）。
- **按鈕比照「作品標籤點擊」既有慣例，用 `data-*` + 事件委派**，不是 inline onclick + window 掛載，理由跟 `filterByCharacter`／`trackGmapsClick` 那次一樣：機台名稱可能含特殊字元，字串拼接的 inline onclick 有拼斷風險，見上方「作品標籤點擊快速篩選」章節。
- **防重複回報**：本機用 `localStorage`（key `machineReport:${machineId}`）記錄最近一次回報的類型跟時間，`MACHINE_REPORT_DEDUPE_DAYS = 3` 天內同一瀏覽器對同一台機台不能再回報（不分「還在」／「不在了」），超過窗口自動視為沒回報過並清掉舊紀錄；`localStorage` 不可用（無痕模式等）時一律放行，不因此擋住按鈕。UI 是樂觀更新：點下去立刻把整組按鈕換成「已回報，謝謝」文字，不等 API 回應，背景 `fetch` 失敗也靜默吞掉——低風險小功能，失敗了使用者路過還會再報一次，不值得為此加重試/錯誤提示的複雜度。
- **資料流刻意極簡**：Worker 收到回報後只做兩件事——KV 累加該機台的 `still`／`gone` 次數（`report:${machineId}`），跟打一次 Discord webhook 通知；不落地成管理頁面、不寫回 Google Sheet（機台資料來源本來就是唯讀的已發布 CSV，前端寫不回去），事後靠 Discord 通知＋人工去 Sheet 改資料，維持現有「人工判斷、手動下架」的流程不變。之後如果 Discord 訊息量大、想要好好整理查歷史紀錄，可以再加一支 Apps Script Web App（部署方式比照專案已有的 `scripts/permanent-id.gs`）把每筆回報寫進 Sheet 新分頁，這次刻意沒做。
- **⚠️ 部署前置作業（我這邊做不到，需要 Gill 手動完成）**：這支 worker 沿用既有 `visitor-counter` 部署／`VISITOR_KV` binding，不用新建 KV namespace；但要重新 `wrangler deploy` 讓 `/report` 路徑生效，且要執行一次 `wrangler secret put DISCORD_WEBHOOK_URL`（貼上 Discord 頻道 webhook 網址）通知才會真的送出——這兩步沒做之前，按鈕點了會樂觀更新本機狀態、但 Worker 那邊收不到資料（`fetch` 404／或 KV 沒寫入），純前端行為（按鈕變成「已回報」）不受影響，只是回報實際上沒送達。
- **部署前置作業已完成**：Gill 已經跑過 `wrangler secret put DISCORD_WEBHOOK_URL`／`wrangler deploy`，`/report` 路徑跟 Discord 通知都手動測試過（`curl` 打 `/report` 回傳 `{"ok":true}`，Discord 頻道也收到通知），Worker 這端確認正常運作；前端這幾支檔案還沒 commit／push，網站上還看不到按鈕。
- **系列欄位（`loc.edition`）只有 0 或 1 個值才顯示按鈕**（Gill 這次對話追加需求）：`utils.js` 新增 `hasSingleEdition(loc)`，用 `,`／`、` 分隔符判斷（比照 `scripts/worker.js` `pickShareImage()` 既有的多值分隔慣例），填了 2 個以上系列（多系列聯名）的機台完全不顯示回報按鈕／已回報文字；沒填欄位視為「不限定」一樣顯示，不是只有剛好填 1 個才算。理由：多系列聯名的機台，回報「不在了」語意不夠明確（不確定是整台機器還是其中一個系列的活動結束），先收斂在單一系列的機台上。
- **按鈕文字／確認文字兩輪微調**：按鈕預設狀態原本只用灰色細框＋灰字（只有 hover 才變色），在淺色背景上幾乎看不見，改成比照 `.type-badge.photocard` 「白底＋色框＋色字」語言、預設就顯色（後來 Gill 又要求拿掉白底，改回 `background: none` 透明背景，保留色框＋色字）；按鈕文字「這台還在／這台不在了」改成「機台還在／機台不在了」。
- **確認文字固定附上「回報表單」連結**：`machineReportedText()` 回傳值尾端固定加一句「如有更多資訊，歡迎填回報表單」，「回報表單」是 `<a target="_blank">`，網址跟 `app.html` 的 `.report-link` 同一份（各自獨立宣告，兩邊沒有共用的地方可以 import）。**這裡有一個討論過程**：一開始構想是「全站累計回報滿 3 次才跳提示」，Gill 一路確認到「全站累計、只跳一次、第 3 次觸發」之後，問了「web 有沒有 dialog 元件」，討論到用原生 `<dialog>` 還是比照站上既有 modal 視覺語言重刻一份，最後 Gill 決定**不要彈窗**、也不要為了「第 3 次」特別換一種文案，簡化成「不管第幾次回報，確認文字都固定同一版、都附回報表單連結」——所以最終沒有實作「累計次數」這套邏輯，單純的文字調整。
- **補上「回報表單」連結的 GA 追蹤**：這個連結是動態插入 DOM 的，原本 `main.js` 綁 `report_click` 的那段 `document.querySelectorAll('.report-link').forEach(...)` 是頁面載入當下才執行一次的靜態綁定，動態產生的元素接不到；改用 `data-report-form-link`／`data-source="machine_report_confirmation"` + 既有的 document click 委派監聽器（新增一段判斷，緊接在 gmaps 連結那段後面），送出同一個 `report_click` 事件、`source` 換成 `machine_report_confirmation`，跟現有 `home_page`／`events_page` 兩個 source 並列，方便 GA 後台分清楚三種來源。`docs/spec.md` 的 `report_click` 那列同步補上第三個 source 值，並新增 `machine_report`（按鈕本身的回報事件）那一列，這份新功能之前漏掉沒登記進事件表。

**⚠️ v35.3 GA4 埋碼盤點補漏（本檔案這次更新時）尚未 commit／push**：對照 `docs/spec.md` 的 GA4 事件表跟實際程式碼逐一比對，找出幾處確實有互動但沒送事件的地方補上；純粹補追蹤，沒有動到任何使用者可見的行為／樣式。動到 `js/events.js`、`js/main.js`、`js/sort-widget.js`、`docs/spec.md`：
1. **`report_click` 沒在活動頁生效**：這個事件原本只綁在 `main.js`（`document.querySelectorAll('.report-link')...`），但 `events.html` 只載入 `events.js`，不載入 `main.js`，所以活動頁桌機／手機兩個「回報表單」連結點了完全不會送出事件——純粹是漏掉，不是刻意不追蹤。在 `events.js` 補一段一模一樣的綁定；順便讓兩邊都補上 `source` 參數（`home_page`／`events_page`）方便在 GA 後台分開看兩頁的回報表單點擊量。
2. **新增 `cross_page_nav_click`**：首頁 FAB（`#eventsLink` → 活動頁）跟活動頁 FAB／logo（`#gachaMapLink`／`.events-logo-link` → 首頁）是兩頁之間唯一的互通入口，先前完全沒有追蹤，看不出使用者是從哪個入口跨過去的。帶 `from_page`／`to_page`／`source`（fab/logo）／`device`；因為是同分頁導頁、點下去馬上就換頁，指定 `transport_type: 'beacon'`，避免事件在換頁過程中被中斷送不出去。
3. **新增 `event_note_link_click`**：活動詳情 Modal 裡「更多資訊：查看」這個外連（`eventDetailNoteRow()`，note 欄位網址）先前沒有事件，同一個區塊的 Google Maps 連結（`gmaps_click`）跟作品標籤（`character_tag_click`）都有，這個漏掉了。跟 `gmaps_click` 一樣掛在 `bindLocationSectionEvents()`，帶 `event_id`（該地點 A 欄 id）／`source`／`device`。
4. **新增 `events_filter_result`**：首頁有 `filter_result`（篩選後符合筆數），活動頁完全沒有對應事件，包含篩出「找不到符合篩選條件的活動」空狀態（`collageEmptyHtml()`）也沒被記錄。比照首頁用 800ms debounce，涵蓋類型／作品／縣市 pill 跟「今日活動」／「有抽卡 / 相卡機」兩顆快速篩選 pill，`result_count = 0` 就對應空狀態。
5. **新增 `search_no_result`／`events_search_no_result`**：`filter_result`／`events_filter_result` 兩邊原本都設計成「沒套用任何篩選時直接跳過」（純關鍵字搜尋、或初始狀態不記錄），這個設計是刻意的（避免雜訊），但副作用是「純搜尋搜不到任何機台／活動」這件事完全沒有留下任何 GA 紀錄。新增這兩個事件補這個缺口，門檻跟既有的 `search`／`events_search` 一致（≥ 2 字、800ms debounce）。
6. **新增 `share_result`／`events_share_result`**：`share_click`／`events_share_click` 只記錄「按下分享按鈕」這個動作本身，不管後面 `navigator.share()` 是被使用者取消、還是真的分享出去、或是退回複製連結，一律不會有第二個事件記錄結果。改成 `navigator.share()`／`clipboard.writeText()` 都接上 `.then()`／`.catch()`，補記 `shared`／`cancelled`（`AbortError`）／`copied`／`failed` 四種結果。
7. **`sort_change`／`geo_permission_result` 補上 `gaPrefix`**：`sort-widget.js` 裡這兩個事件先前是寫死的固定字串，沒有跟其他 `sort-widget.js` 事件（`sort_panel_open`／`sort_panel_close` 等）一樣套用 `gaPrefix`，導致首頁跟活動頁的排序／定位授權結果在 GA 後台混在同一個事件名稱裡分不開，只能再靠 page_location 自己篩。改成套 `gaPrefix`：首頁 `gaPrefix` 預設是 `'sort'`，事件名稱維持 `sort_change`／`geo_permission_result` 不變（相容既有 GA 資料）；活動頁 `gaPrefix` 是 `'events_sort'`，改名成 `events_sort_change`／`events_sort_geo_permission_result`。**⚠️ 這代表活動頁排序相關事件在 GA 裡會出現名稱斷點：v35.3 之前的活動頁排序資料留在舊的 `sort_change`／`geo_permission_result`，之後才會出現在新名稱底下，統計歷史趨勢時要注意這個切點。**

`docs/spec.md` 的 GA4 事件表（主表＋「活動行事曆專屬」表）已同步補上以上新事件；GA4 後台「自訂定義」跟 Notion「GA4 事件追蹤表」尚未登記，見 `spec.md` 該段待辦註記。

**⚠️ v35.2 月曆週次日期列 sticky 固定 ＋ 斷點統一與 events 頁 topbar 結構精簡 ＋ Icon 系統整理 ＋ 卡片格線 min-width 調整 ＋ 側邊欄卡片格線與詳情關閉鈕 sticky 化 ＋ 訪客 banner 顯示 bug 修正 ＋ 地圖聚合點清單／活動詳情 Modal／拼貼列表版面細部樣式修正 ＋ Lightbox／作品標籤點擊篩選共用化 ＋ 月曆左右留白斷點化／topbar 捲動例外／day-events-panel 斷點統一與視覺語言統一 ＋ 日期 hover 外匡上邊框二次修正／events-bar 左邊統一 12px 內縮／day-events-body 左右 padding 調整／全站關閉鈕 icon 統一 ＋ day-events-panel 卡片清單 ≤991px 改依寬度自動算欄數 ＋ 總覽·拼貼格狀可左右滑動 bug 修正（本檔案這次更新時）尚未 commit／push**：
1. **月曆週次日期數字列（`.events-week-daynums`）捲動時改為 sticky 固定**（2026/09/11，補記——原本只更新了 Notion「設計迭代紀錄」v35.2，本機文件當時漏補）：某一週活動較多、展開「全部顯示」後 `.events-day-grid` 內部會捲動，這排日期數字（1、2、3...）原本只是該週最上面一列，捲動後跟著捲走，畫面上只剩外層固定的 `.events-weekday-row`（週幾：一二三四五六日），看不出目前捲到的是幾號。在 `.events-week-daynums` 加上 `position: sticky; top: 0; z-index: 2;`，讓每週的日期數字列在該週往下捲動時黏在捲動容器（`.events-day-grid`）頂端，捲到下一週時再自然換成下一週的日期列（sticky 標準行為，不需額外寫 JS），`z-index` 蓋過同一週往下捲的 `.events-week-bars`／`.events-week-hover-col`；已確認中間沒有祖先元素設 `overflow` 會擋到 sticky 生效。動到 `events.css`。
2. **全站手機／桌機切換斷點 900px／901px → 991px／992px**：`style.css`／`events.css` 對應的 media query、`js/scroll.js`／`js/events-scroll.js` 的 `MOBILE_BREAKPOINT`、`js/events.js`／`js/filters.js`／`js/map.js` 的 `matchMedia`，全部從 900px（`min-width:901px`）改成 991px（`min-width:992px`）；`events.css` 原本另一組獨立的 959px 斷點（`.events-filter-row` 換行、toolbar 全寬）一併併入 991px，不再是獨立數字。720px／721px（events 頁行事曆／卡片列版型切換用）跟 450px（窄螢幕 header 標題，見下方第 5 點）維持獨立，沒有一起改。
3. **排序按鈕（`.sort-btn`）視覺改版**：桌機樣式從純文字按鈕改成跟篩選 pill（`.filter-pill`）一致的白底圓角膠囊（`background`／`border-radius`／`padding` 對齊），991px 以下進一步收成 44×36px 純 icon 按鈕：文字 label（`.sort-label`）隱藏、原本的下拉 chevron 換成新增的排序 icon（`svg icon/sort_20dp_000000_FILL0_wght400_GRAD0_opsz20.svg`），`js/sort-widget.js` 的 render template 新增 `.sort-icon-mobile`，`style.css` 用 `display:none`／`display:inline-flex` 依斷點切換兩顆 icon，desktop 下拉 panel 開合行為未變動。
4. **events 頁 header／toolbar／searchbar 結構精簡到跟機台首頁完全對稱**：
   - 拿掉 `.events-topbar-controls` 外層 wrapper，`.toolbar`（原 `.events-search-row`，改成直接沿用首頁共用的 `.toolbar` class）、`.events-filter-row`、`.events-weekday-row` 現在都直接是 `#eventsTopBar` 的子元素，各自管自己的左右 padding／margin，不再靠外層 wrapper 統一撐開，結構比照 app 頁 `#topBar` 底下 `header`／`.toolbar`／`.filter-bar` 平行並列的模式。
   - 拿掉 `.quick-filter-group` wrapper，「今日活動」／「有抽卡 / 相卡機」兩顆快速篩選 pill 改成 `.filter-scroll` 的獨立子元素（跟類型／作品／縣市 pill 同層），`js/events.js` 的 `positionQuickFilterGroup()` 改成分別抓 `#eventsTimeFilterPill`／`#eventsMachineFilterPill` 各自搬移，不再靠 wrapper 一次搬；兩顆現在能吃到 `.filter-scroll` 的 `gap:12px`，跟其他 pill 間距一致。
   - `.events-filter-row` 撐開跟下一列距離的方式從 `margin-bottom` 改成 `padding-bottom`（比照 `.filter-bar` 的模式），基礎規則合併成 `padding: 0 16px 16px 16px`；拼貼列表模式／月曆導覽列顯示中這兩條依情境覆寫規則同步從 `margin-bottom` 改成 `padding-bottom`。
5. **`.filter-bar` 991px 以下 padding 對齊成 `0px 16px 16px 16px`**（原本左右不對稱是 `0px 16px 24px 24px`），並移除一條 `body:not(.map-view) .filter-bar { padding-bottom: 0 }` 的舊覆寫——這條原本是列表模式下故意把 `.filter-bar` 底部 padding 歸零、只靠 `#gridView` 自己的 `padding-top` 撐開間距，避免疊加過大；拿掉後列表模式的 `.filter-bar` 底部間距變成 padding-bottom（16px）＋ `#gridView` 的 padding-top（12px）＝28px，跟地圖模式一致吃到完整 padding。
6. **`.logo h1` 欄狀排列提前到 991px 以下生效**：原本只有 450px 以下（窄螢幕如 iPhone SE）才會把副標題「抽卡機在哪」獨立成一行、字級縮到 16px，現在 991px 以下就套用；原本 450px 專屬的區塊已刪除（涵蓋範圍被 991px 完全包含，留著會重複）。
7. **月曆橫幅（`.events-bar`）排序改成「剩餘天數優先」（行動導向，補記——原本只更新了 Notion「設計迭代紀錄」v35.2，本機文件當時漏補）**：`weekEventBars()` 新增 `daysUntilEnd(group, today)` helper，算「活動結束日 - 今天」還剩幾天（只看年/月/日，忽略時分秒），已結束的回傳 `Infinity` 排到最後面；排序 key 從原本的「開始欄位小的優先、同開始欄位橫跨天數多的優先」改成「剩餘天數少的（快結束的）優先」，剩餘天數相同才退回原本那組橫向排列規則當 tie-break。注意這個排序會直接餵給下面的貪婪打包演算法（`rowLastCol` 那段）決定塞進第幾列，所以跟原本「開始日早、跨天長優先」比起來，行動導向排序可能會讓同一週用到比較多列（打包效率變差，較容易需要點「全部顯示」）——這是刻意接受的取捨，換取「快沒了的活動優先被看到」。`renderWeekRow(week, today, weekKey, groups)` 呼叫 `weekEventBars()` 時新增傳入 `today`。動到 `js/events.js`。

8. **Icon 系統整理：修正幾處對不上規範的圖示、抽出跨檔案重複的 inline SVG 為共用常數**（`js/utils.js` 新增 `BTN_EXPAND_ICON_SVG`／`SHARE_BTN_ICON_SVG`／`CAROUSEL_CHEVRON_ICON_SVG` 三個常數）：
   - `app.html` 列表 view-toggle 按鈕未啟用（outline）狀態的 icon，原本是另一款舊版 `grid_view` 變體，換成正確的 `grid_view_20dp_000000_FILL0_wght400_GRAD0_opsz20`；啟用狀態的 icon 本來就正確，地圖 icon 兩顆也本來就正確，這次順便逐一核對過
   - 搜尋框清除（✕）按鈕圖示（`app.html`／`events-app.html` 共 4 處）從舊版 24×24 stroke 圓圈打叉字元圖示，換成 `cancel_20dp_000000_FILL1_wght400_GRAD0_opsz20`，尺寸從 16px 放大成 20px（跟其他 icon 統一）；放大鏡 search icon 本來就已經是 `search_20dp_000000_FILL0`，沒有動
   - `.btn-expand`（`grid.js`／`events.js` 各一處，逐字元重複貼了兩份 `expand_content_20dp_000000_FILL1` 的 inline SVG）、`.popup-share-btn`（`main.js`／`map.js`／`events.js` 三處，逐字元重複貼了三份 `share_20dp_000000_FILL0` 的 inline SVG，path 本身一直是對的，純粹是重複）都抽成 `utils.js` 共用常數，四個呼叫端改成 import 使用同一份字串，理由跟既有的 `MACHINE_TYPE_BADGE_ICON` 共用慣例一致——這幾個都是零依賴的純字串，不受「`events.js` 不能 import `main.js` 那條鏈」的限制
   - 新增 `CAROUSEL_CHEVRON_ICON_SVG`（`chevron_backward_20dp_000000_FILL0`，14px）取代 `.carousel-btn`（`main.js`／`map.js`／`grid.js`／`events.js` 四處）原本寫死的 unicode `‹`／`›` 字元；next 按鈕新增 `.carousel-btn-next` class，靠 CSS `transform: scaleX(-1)` 把同一顆 icon 鏡射成反方向，不用另外準備一顆 forward icon（`style.css` 的 `.carousel-btn` 移除原本給文字用的 `font-size`／`font-weight`，新增 `.carousel-chevron`／`.carousel-btn-next` 對應規則）
   - `.related-machines-nav-btn`（`events.js`，相關機台卡片列左右箭頭）原本用另一款不在 `svg icon` 資料夾裡的舊版「chevron_right」風格 SVG，改用跟 carousel 共用的同一顆 `CAROUSEL_CHEVRON_ICON_SVG`；因為這顆 icon 原生朝左，鏡射方向要反過來——`events.css` 從「prev 鏡射」改成「next 鏡射」（`.related-machines-nav-btn.next svg { transform: scaleX(-1); }`），拿掉原本 prev 按鈕上 inline 的 `style="transform:scaleX(-1)"`
   - `.events-nav-group`（`events-app.html`，月曆上/下個月按鈕）原本用 stroke 風格的 `<polyline>` icon（24×24 viewBox），換成同一顆 `chevron_backward` fill 風格 icon；這裡是寫死在 HTML 裡（不是 JS 動態產生），沒辦法直接 import `utils.js` 的常數，改成把同一段 path 字串直接貼進 HTML，下個月按鈕新增 `.events-nav-next` class 搭配 `events.css` 新增的鏡射規則
   - `app.html` 的 FAB（`#eventsLink`，連到活動頁）icon 從 `calendar_today` FILL1 換成 `local_activity_20dp_000000_FILL1`；`events-app.html` 的 FAB（`#gachaMapLink`，連到機台頁）核對後本來就已經是 `playing_cards_16dp_000000_FILL1`，沒有動
   - 這一輪也逐一核對過幾處「看起來像沒改過」的地方，確認都已經跟目標 icon 一致、不用動：`filter-widget.js`／`sort-widget.js` 的 `pill-chevron`（`arrow_drop_down` FILL1）、`sort-widget.js` 的 991px 以下 `.sort-icon-mobile`（`sort_20dp` FILL1）、`grid.js` 的 `.btn-gmaps`（`map_pin_heart` FILL0）、`events-app.html` 月曆按鈕的 `.calendar-icon-outline`/`.calendar-icon-fill`（`calendar_today` FILL0/FILL1）、`events.js` 的 `.events-multi-pill`／`.events-bar-multi-badge`（`map_pin_heart` FILL1）、`events-app.html` 總覽 icon（固定用 `list_20dp` FILL0，本來就沒有 active/inactive 切換）
   - 動到 `js/utils.js`／`js/main.js`／`js/map.js`／`js/grid.js`／`js/events.js`／`style.css`／`events.css`／`app.html`／`events-app.html`，共 9 個檔案

9. **`events-app.html` 補齊 `app.html` 原本就有的「訪客 banner 一開始保留固定高度」寫法，順帶修掉一個訪客數在活動頁永遠不會顯示的 bug，`#topBar` 滑動隱藏機制拿掉 `ResizeObserver`**：`app.html` 的 `#visitorBanner` 本來就是 inline `visibility:hidden`；`events-app.html` 當初複製貼上時留成 `display:none`，兩頁各自演化留下的不一致。**實際的 bug**：`js/visitor.js`（app／events 兩頁共用同一份腳本）抓到人數成功時只會把 `visibility` 切成 `visible`，從來沒有動過 `display`——`events-app.html` 行內的 `display:none` 優先度比 `.visitor-banner` class 的 `display:flex` 高、且不受 `visibility` 影響，導致這顆 banner 在活動頁不管 API 有沒有抓到人數都一直是 `display:none`，實際上永遠顯示不出來（只有失敗分支 `style.display='none'` 是「有效」的，成功分支形同虛設）。修法是把 `events-app.html` 也統一改成 `visibility:hidden`：CSS 預設的 `display:flex` 讓它從一開始就佔著版面空間，只是視覺上先隱形，不會在人數抓取成功那一刻才讓 `#topBar`/`#eventsTopBar` 高度跳動；`js/visitor.js` 本身邏輯不用改（成功切 `visibility`、失敗切 `display:none`，本來就是這樣寫）。連帶地，`js/scroll.js`（app 頁 `#topBar` 滑動隱藏機制）原本用 `ResizeObserver` 持續盯著 `#topBar` 尺寸變化，理由是「banner 非同步出現、螢幕旋轉」——banner 兩頁都保留固定高度後，這個持續監看的理由消失（KADO 也不需要處理裝置旋轉這種情境），拿掉 `ResizeObserver`，`updateTopBarHeight()` 改成只在初始化跟 `window resize`（手機/桌機斷點切換）時重算，之後如果又出現會動態改變 `#topBar` 高度的內容，再視情況加回。詳見下方「訪客計數 Banner」「#topBar 滑動隱藏」兩個章節的 v35.2 條目。動到 `events-app.html`／`js/scroll.js`（`app.html`／`js/visitor.js` 這次沒有實際改動，原本就是對的）。
10. **「最後更新」時間轉換/比較邏輯、抽卡機/相卡機 type-badge 圖示，搬進 `utils.js` 共用，拿掉跨檔案重複**：
    - `to24Hour()`／`parseUpdateDate()`／新增的 `buildLastUpdatedText(machineRaw, eventsRaw)`（把「比較兩欄時間戳、組出顯示文字」也一併收進共用函式，不只是搬底層轉換函式）搬進 `utils.js`；`js/main.js`（`loadFromSheet()`）跟 `js/events-header.js`（`loadLastUpdated()`）原本各自複製一份逐字元相同的邏輯，改成都 import 這一份，之後要調整比較規則只要改一處。詳見下方「最後更新元素」章節 v35.2 條目
    - `MACHINE_TYPE_BADGE_ICON`（圖示 SVG）跟新增的 `machineTypeClass(type)`（回傳 `gacha`/`photocard`）搬進 `utils.js`；原本同一段逐字元相同的抽卡機/相卡機 inline SVG 字串，各自獨立複製在 `grid.js`（1 處）／`main.js`（1 處）／`map.js`（3 處，含 marker 顏色 class，但 marker 圖示本身用的 `MARKER_ICON_SVG` 刻意不共用——那是屬性不同的 divIcon 專用版本，見「地圖 Marker 設計」）／`events.js`（原本自己也獨立宣告一份 `MACHINE_TYPE_BADGE_ICON` 常數），改成四個檔案都 import `utils.js` 這一份；順手在常數上方補了圖示來源註解（`抽卡機` → Material Symbols `playing_cards`、`相卡機` → `photo_camera`，皆 FILL1），對齊 `svg icon/` 資料夾裡實際下載的原始檔名。詳見下方「共用工具搬遷到 utils.js」章節新增條目
    - `js/main.js` 的 `openGridModal()` 原本 `getEndingBadge(loc.limited)` 在同一段模板字串裡被呼叫兩次（判斷式一次、內容組字串又一次），順手改成只算一次存進變數，跟 `grid.js`/`map.js` 既有寫法一致
    - 動到 `js/utils.js`／`js/main.js`／`js/events-header.js`／`js/grid.js`／`js/map.js`／`js/events.js`，共 6 個檔案

11. **機台首頁列表／events 頁「總覽·拼貼格狀」／「總覽·列表」三處卡片格線，min-width 統一調整為 330px**（Gill 這次是直接口頭指定數值，不是先觀察破版才回推）：
    - `style.css` 的 `.grid`（機台首頁列表模式容器，`js/grid.js` `renderGrid()` 用）：`grid-template-columns: repeat(auto-fill, minmax(300px, 1fr))` 改成 `minmax(330px, 1fr)`
    - `events.css` 的 `.collage-list-grid`（events 頁「總覽·列表」容器，沿用首頁 `.loc-card-grid` 同一套卡片語言，見「拼貼列表卡片沿用首頁 `.loc-card-grid`」章節）同步從 `minmax(300px, 1fr)` 改成 `minmax(330px, 1fr)`——這是跟 `.grid` 完全獨立的宣告，改其中一個不會自動同步到另一個，這次是兩處都手動各自改過
    - `events.css` 的 `.collage-card`（「總覽·拼貼格狀」卡片，見「拼貼格狀排版」章節，是跟上面「列表」不同的排版）原本完全沒有 `min-width`，寬度單純由 `.collage-grid-col`（`flex: 1 1 0`）動態分配；這次先試著直接加 `min-width: 614px`，上線一測發現破版（欄數還是固定斷點決定，4 欄 × 614px 遠超過容器寬度，卡片被撐爆造成橫向溢出），跟 Gill 確認後改成 330px，同時把下面這條欄數邏輯一起修正
    - 因為「拼貼格狀」卡片新增了 min-width，原本 `js/events.js` 的 `collageColumnCount()` 純靠螢幕寬度分四段固定斷點（420/720/1100px）給 1～4 欄的邏輯，會在欄數跟卡片最小寬度對不上時把卡片撐爆容器（見上一點的 614px 破版案例；即使改成 330px，桌機寬度只要 >1100px 就固定切 4 欄，在較窄的桌機視窗下仍可能不夠寬）——改成量測 `#eventsCollageGrid` 實際可用寬度，用「能塞下幾個卡片 min-width 欄」反推欄數（新增 `COLLAGE_CARD_MIN_WIDTH = 330`／`COLLAGE_MAX_COLS = 4` 兩個常數，`Math.floor(width / COLLAGE_CARD_MIN_WIDTH)`，下限 1 欄、上限維持原本的 4 欄），取代原本三個寫死的斷點數字；resize debounce（150ms）邏輯不變，欄數變動時才重新分欄重繪
    - 動到 `style.css`／`events.css`／`js/events.js`，共 3 個檔案

12. **地圖側邊欄／bottom sheet 卡片清單改成依寬度自動算欄數（CSS Grid），並修掉因此波及機台詳情面板的 bug**：`.location-list` 原本是普通 block 排版，卡片一律單欄堆疊，不管桌機側邊欄（固定 400px 寬）還是手機 bottom sheet（滿版寬）都一樣；改成 `display: grid; grid-template-columns: repeat(auto-fill, minmax(330px, 1fr)); gap: 20px;`（`min-width` 跟首頁 `.grid`／events 頁 `.collage-list-grid` 統一，見上方第 11 點），手機版橫向夠寬（平板、手機橫向）時會自動變多欄。**踩到的坑**：`#locationList` 這個容器是列表卡片跟機台詳情共用的同一個 DOM 節點——`buildDetailContentHtml()` 產生的詳情內容是好幾個獨立的 top-level 區塊（badge 列、`.modal-header`、`.modal-info-section`...），不是包在單一 `.loc-card` 裡，直接把 grid 套在 `.location-list` 上會連這些詳情區塊都被拆成多欄，版面跑掉。修法：grid 規則收進新增的 `.location-list.card-grid` 修飾 class，只在真的渲染「卡片清單」的地方（`openDesktopSidebar`／`renderDesktopDefaultPanel`／`renderMobileSheetContent` 列表分支）加上這個 class，渲染詳情內容或空清單提示文字時移除，讓它退回單欄 block 排版。動到 `style.css`／`js/map.js`。
13. **機台詳情面板的關閉鈕改為 sticky 固定，不再隨內容一起被捲走**：`buildDetailContentHtml()` 原本把 badge 列（作品／倒數）跟關閉鈕包在同一個 `flex` row 裡，整段都是 `#locationList` 捲動內容的一部分，內容一長，往下捲就會連關閉鈕都捲出視野。改法：badge 列（`.modal-badge-row.detail-badge-row`）維持原本 block 排版跟著內容捲動；關閉鈕另外包一層 `.detail-close-btn-wrap`（`position: sticky; top: 0; height: 0;`），`height: 0` 讓它不佔版面高度、不會把 badge 往下推，`sticky` 讓它捲動時黏在可捲動區塊（`.map-scroll-wrapper`）頂端；按鈕本身（`.popup-close-btn.detail-close-btn`）再用 `position: absolute; top: 0; right: 16px;` 相對這層 wrapper 定位到右上角，兩個 class 疊加是為了蓋過基礎 `.popup-close-btn` 的 `top:4px; right:4px`，不受兩條規則在檔案裡先後順序影響。外觀刻意不加圓底背景／陰影，維持跟 `.grid-modal-close` 一致的純圖示、透明底樣式。動到 `js/map.js`／`style.css`。

14. **地圖聚合點清單／活動詳情 Modal／拼貼列表版面留白間距／月曆多地點徽章，一批細部樣式修正（這次對話逐項討論確認後修正，非先觀察破版才回推）**：
    - `.cluster-popup-item.selected .cluster-popup-item-name` 補上 `font-weight: 700`，選中項目標題字重加粗，跟未選取項目做出區別
    - `.cluster-popup-item-name`／`.cluster-popup-item-sub` 補上 `overflow: hidden; text-overflow: ellipsis;`，`.cluster-popup-item-text` 補上 `min-width: 0`（flex item 預設 `min-width: auto` 會撐開容器、擋住 ellipsis 生效），文字過長時單行截斷顯示 `...`，不再撐破卡片版面
    - 活動詳情 Modal（`js/events.js` `openEventDetailModal()`）的 badge 列原本用 `events-detail-badge-row` 這個 class，但 `events.css` 沒有任何對應規則（純瀏覽器預設 `<div>`，沒有 padding／flex／margin），導致上下沒有留白，跟機台詳情面板（`map.js`，用共用的 `.modal-badge-row.detail-badge-row`）視覺不一致；改成沿用同一組共用 class（`display:flex; gap:4px; margin-bottom:12px`），對齊留白
    - `.character-chips-toggle`（活動詳情「+N 個作品」展開／收合按鈕）補上遺漏的 `font-family: 'Noto Sans TC', sans-serif;`——`<button>` 元素不會自動繼承 `body` 設的字型，站上其他按鈕都各自補了這行，這顆漏補，之前吃到瀏覽器預設系統字型跟其他按鈕不一致
    - `.events-bar-multi-badge`（月曆橫幅「N 地點」徽章）配色從跟著分類變色（`background: var(--bar-color); color: var(--fill-white);`，吃跟左側色條、`.events-bar-cat` 同一顆 `--bar-color` 變數）改成固定白底藍字（`background: var(--fill-white); color: var(--fill-blue);`），跟卡片版／詳情彈窗用的 `.events-multi-pill` 統一；`margin-left:auto` 靠右定位、`border-radius:9999px` 藥丸形、`flex-shrink:0` 等版型邏輯不變，只改配色
    - events 頁「總覽·列表」（`.collage-list-grid`）左右留白原本直接繼承外層 `.events-page-body` 固定 20px（不分斷點），改成 `.collage-list-wrap` 自己用「負 margin（`margin: 0 -20px`）抵銷外層固定 20px、再依斷點補回 `padding-left`／`padding-right`」的方式控制：桌機（>991px）20px、手機（≤991px）16px，跟機台首頁 `.toolbar`／`.filter-bar`／`#gridView` 同一套「桌機 20、手機 16」的慣例對齊；只局部影響「總覽·列表」，不動到「總覽·拼貼格狀」（`.events-collage-grid`，本來就左右貼邊不需要 padding，見下面一點）跟月曆檢視
    - 上方間距原本 `.collage-list-wrap` 跟 `.collage-list-grid` 各自都有一份 `padding-top: 8px`，桌機疊加成 16px（重複）；改成只由 `.collage-list-grid` 一處控制：桌機（>991px）8px、手機（≤991px）0px，斷點從原本的 720px 改成 991px（跟上面左右留白統一）；`.collage-list-wrap` 自己另一條跟 fixed top bar 高度補償相關的 `padding-top: calc(8px + var(--events-top-bar-height, 0px))`（≤991px，功能性，避免內容被固定在頂部的 bar 蓋住）維持不動，兩者是分開的機制
    - `.events-collage-grid`（「總覽·拼貼格狀」，本身已經是不分斷點都左右貼邊、不用 padding 的既有設計，這次沒改動這部分）在 ≤991px 的 `padding-top` 原本是 `var(--events-top-bar-height, 8px)`——只有 top bar 高度變數還沒量到時才 fallback 用 8px，量到之後就整個換成 top bar 高度、沒有額外視覺間距；改成 `calc(8px + var(--events-top-bar-height, 0px))`，補償高度之上固定再加 8px 視覺間距，跟 `.collage-list-wrap` 的邏輯一致；>991px 桌機沿用原本固定的 `padding-top: 8px` 不變（原本就有，不用改）
    - 動到 `style.css`／`events.css`／`js/events.js`，共 3 個檔案

15. **Lightbox 開關邏輯搬進 `utils.js` 共用；作品標籤點擊篩選（`filterByCharacter`）／Google Maps 點擊追蹤（`trackGmapsClick`）改用 `data-*` 屬性＋事件委派，拿掉 `window` 掛載**（這次對話討論「main.js／events.js 哪些元件真正共用」時，找出兩個可以進一步收斂的重複點，分兩輪改完）：
    - **Lightbox**：`utils.js` 新增 `createLightbox(overlayId, imgId)`，回傳 `{ open, close }`，把「設定 `img.src`、toggle overlay 的 `show` class」這段機台版／events 版逐字元重複的核心邏輯抽出來共用；`main.js`／`events.js` 各自呼叫 `createLightbox('lightbox','lightboxImg')`／`createLightbox('eventLightbox','eventLightboxImg')` 拿到各自獨立的物件，`openLightbox()`／`closeLightbox()`／`openEventLightbox()`／`closeEventLightbox()` 內部改呼叫 `lightbox.open()`／`lightbox.close()`。**只抽開關這一段**，GA 事件、`window.closeLightbox` 掛載（機台版）、`addEventListener` 綁定（events 版）都刻意留在各自檔案——這些跟兩頁本來就不同的「動態 HTML 呼叫 JS」綁定慣例綁在一起，不適合、也沒必要一起搬，詳見下方「圖片放大 Lightbox」章節的 v35.2 補記。
    - **`filterByCharacter`／`trackGmapsClick` 改用 `data-*` + 委派**：這兩個函式原本因為 `main.js`／`grid.js`／`map.js` 動態產生的 HTML 字串裡都要跨檔呼叫，掛在 `window` 上，由 inline `onclick="filterByCharacter('${loc.character.replace(/'/g,"\\'")}',...)"`／`onclick="trackGmapsClick(...)"` 呼叫。改法：`main.js`（grid modal）、`map.js`（地圖詳情面板）、`grid.js`（列表卡片）產生的作品按鈕／Google Maps 連結，全部改成純資料屬性——作品按鈕用 `data-character`／`data-machine-id`／`data-source`，gmaps 連結用 `data-gmaps-track`／`data-machine-id`／`data-source`，拿掉 inline `onclick`；`main.js` 既有處理 lightbox／grid carousel 的那個 document click 委派監聽器（見「Lightbox（grid）」段落）新增兩段判斷，比對 `.popup-character-link[data-character]` 呼叫 `filterByCharacter()`、比對 `[data-gmaps-track]` 呼叫 `trackGmapsClick()`（gmaps 連結本身是原生 `<a target="_blank">`，這裡只負責送 GA，不 `preventDefault`，連結照常開新分頁）；`trackGmapsClick`／`filterByCharacter` 改成 `export`，`window.trackGmapsClick = trackGmapsClick`／`window.filterByCharacter = filterByCharacter` 兩行拿掉。**順便解決的副作用**：原本 `character.replace(/'/g, "\\'")` 手動跳脫單引號、防止作品名稱含英文撇號時把 inline onclick 字串拼斷的風險，改成資料屬性後完全不存在——不是拼接 JS 程式碼字串，是單純的 HTML 屬性值。`map.js`／`grid.js` 不再需要 inline onclick，也就不用額外 `import` 這兩個函式，委派監聽器統一掛在 `main.js`、涵蓋整個 `document`，不管哪個模組產生的 HTML 都接得到。**`shareLocation`／`trackEventTitleClick`／`closeDetailPanel` 等其餘 inline onclick ＋ window 掛載的函式這次刻意沒有一起改**——評估過風險與效益後，這次只做範圍最小、驗證最容易的兩個函式；全站還有好幾處類似模式，之後如果要繼續收斂，建議整批一次規劃，不要分散著改，詳見下方「作品標籤點擊快速篩選」章節的 v35.2 補記。
    - 動到 `js/utils.js`／`js/main.js`／`js/map.js`／`js/grid.js`，共 4 個檔案

16. **月曆 `.events-day-grid`／`.events-weekday-row` 左右留白依斷點套用 16px／20px、捲動時排除 topbar 隱藏機制、日期 hover 外匡 z-index 修正**（這次對話逐項討論確認）：
    - `.events-day-grid`（月曆格線本身，含日期數字列跟 event bar，是有背景色／圓角的白底卡片，不是單純捲動容器）跟 `.events-weekday-row`（星期列：一二三四五六日）左右留白統一依斷點切換：≤991px 16px、>991px 20px，對齊 `.events-page-body`／`.filter-bar` 既有的「桌機 20、手機 16」慣例。**踩到的坑**：`.events-day-grid` 一開始用「外層負 margin 抵銷、自己補 padding」這招（跟「總覽·列表」`.collage-list-wrap` 同一套手法）試著改，結果背景色卡片看起來還是貼齊邊緣沒有留白——因為這招只對「本身沒有可視背景」的元素有效，padding 是畫在背景框「內側」，對於本身就是背景／卡片的元素，改 padding 只會讓內容往內縮、背景範圍不變，視覺上等於沒留白；改回直接用負 `margin-left`／`margin-right: -4px`（僅 ≤991px），讓背景卡片本身的邊界跟著內縮，才是正確做法。`.events-weekday-row` 則單純用 `margin: 0px 16px`（≤991px）／原本的 20px（>991px）控制。動到 `events.css`。
    - **月曆模式下，`#topBar` 捲動隱藏機制改成一律保持可見，不再跟手機版一樣滑動就收合**：原因是月曆內部（`.events-day-grid`）自己有獨立捲動，使用者往下捲月曆內容時，`js/events-scroll.js` 原本的手機版邏輯會把整個 topBar（含月曆導覽列）一起滑走，體驗上很奇怪；一開始考慮「整個 topbar 一律 fixed（不分模式）」，跟 Gill 討論後拍板「只有月曆模式」這個範圍最小的做法，非月曆模式（拼貼／列表）維持原本手機版滑動隱藏行為不變。改法：`js/events-scroll.js` 新增 `const ALWAYS_VISIBLE_TARGET_ID = 'eventsDayGrid'`，`handleScroll()` 判斷式從 `if (!isMobile())` 改成 `if (!isMobile() || el.id === ALWAYS_VISIBLE_TARGET_ID)`，捲動來源元素是月曆格線本身（`#eventsDayGrid`）時就直接視為「不隱藏」分支，不受 `isMobile()` 影響。動到 `js/events-scroll.js`。
    - **日期 hover 外匡（`.events-week-hover-col`）上邊框被 sticky 日期數字列蓋住**：`.events-week-daynums`（每週日期數字列，見上方第 1 點）為了 sticky 捲動正確顯示設了 `z-index: 2`，蓋過了同一格內、原本沒設 z-index（預設 0）的 hover 外匡（`box-shadow` 畫出的四邊外框），導致 hover 時只看得到左右下三邊、上邊被日期數字列的背景擋住。修法：`.events-week-hover-col` 補上 `z-index: 3`，蓋過 `.events-week-daynums` 的 `z-index: 2`。動到 `events.css`。

17. **day-events-panel（點擊日期展開的當日活動 drawer）批次調整：斷點統一、卡片視覺語言沿用 `.loc-card-grid`、排序對齊月曆、標題置中、`.selected` 狀態手動清除、桌機展開時的留白對齊**（這次對話逐項討論確認）：
    - **斷點從 720／721px 統一併入全站 991／992px**：`day-events-panel` 原本用自己一組獨立的 `@media (max-width: 720px)`／`(min-width: 721px)` 切換「桌機右側推開版面」跟「手機 bottom sheet」，跟全站已經統一過的 991／992px 斷點（見上方第 2 點）不一致；改成 `@media (min-width: 992px)`（桌機推開）／`@media (max-width: 991px)`（bottom sheet：`.day-events-overlay`／`.day-events-panel`／`.day-events-header`／`.day-events-title`／`.day-events-close`／`.day-events-body`）。同一輪順手拿掉另一條完全無關、但剛好也是 `@media (max-width: 720px)` 的舊規則（`.events-day-num-cell` padding／`.events-bar` 字級／`.events-week-toggle` 字級三條），這條在上一輪 v35.2（見第 2 點筆記）就已經注意到但當時判斷跟 day-events-panel 無關、刻意沒有一起動，這次 Gill 確認「一起拿掉」才清掉。動到 `events.css`。
    - **卡片改沿用機台首頁／events 頁「總覽·列表」共用的 `.loc-card-grid` 視覺語言**：原本 panel 卡片是獨立一套 `.events-card`／`.events-card-body`／`.events-card-pills`／`.events-card-title`／`.events-card-venue`／`.events-card-period` class，跟其他地方的卡片長得不一樣、也沒有已結束灰階／tag 資訊。改成沿用 `.loc-card-grid`（`type-badge`、已結束 `.is-ended` 灰階、角色／城市／場地 `.tag`），但**刻意不顯示「詳情」按鈕**（panel 本身點卡片就是開詳情 modal，不需要再一顆按鈕）。做法：從 `eventListCardHtml()`（「總覽·列表」用，含詳情按鈕）抽出共用的 `locCardTopHtml(group, { ended, badge })`（回傳 `.card-top` 內容：type-badge／multi-pill／機台徽章／即將結束或已結束徽章、`.card-name`、`.card-limited`、`.card-tags`），`eventListCardHtml()` 呼叫 `locCardTopHtml()` 後再自己接 `.card-actions`／「詳情」按鈕；panel 專用的 `eventGroupCardHtml(group)` 改成只呼叫 `locCardTopHtml()`、不接按鈕，包一層 `.loc-card-grid`（帶 `is-ended`／`selected` 修飾 class）。`openDayEventsPanel()` 的點擊委派選取器同步從 `.events-card` 改成 `.loc-card-grid`，並補上 `is-clickable` class。動到 `js/events.js`／`events.css`（刪掉整組 `.events-card` 系列 dead rule，只保留外層 wrapper `.events-card-list`，新增 `.events-card-list .loc-card-grid.selected` 選取樣式）。
    - **panel 卡片排列順序對齊月曆橫幅排序**：`eventsForDate(date)` 原本沒有明確排序（單純 filter），跟月曆 bar 的「剩餘天數優先」排序（見上方第 7 點 `daysUntilEnd()`）不一致，同一天在月曆上跟在 panel 清單裡可能順序不同，容易讓人誤會是兩份不同資料。改成 `eventsForDate()` 也用同一組排序 key（`daysUntilEnd(a, today) - daysUntilEnd(b, today)`，相同才退回開始欄位小的優先、同開始欄位橫跨天數多的優先），跟 `weekEventBars()` 共用同一套「行動導向」排序邏輯（單日情境下，貪婪列排包裝順序等同排序順序，兩處視覺一致）。動到 `js/events.js`。
    - **標題（`.day-events-title`）改為置中，桌機／手機共用同一套定位**：原本桌機／手機可能各自處理，改成 `.day-events-header` base 規則統一用 `display:flex; align-items:center; justify-content:center; position:relative;` 置中標題，`.day-events-close` 改用 `position:absolute; right:16px; top:50%; transform:translateY(-50%);` 相對 header 定位到右側（比照 `.filter-sheet-header` 既有的置中標題語言）；≤991px 的 media block 因此瘦身，只保留手機專屬的字級／padding／關閉鈕右側偏移微調，不再重複宣告 `justify-content`／`position:absolute` 這類已經搬進 base 規則的內容。動到 `events.css`。
    - **從 panel 點卡片開 modal、關閉 modal 後，panel 卡片的 `.selected`（藍色外匡）狀態沒有跟著消失**：day-events-panel 的卡片清單不是靠 `renderAll()` 整批重繪的（專案既有慣例，見 CLAUDE.md「手動 DOM 同步」相關章節），`.selected` class 是點擊當下手動加上去的，關閉 modal 的既有流程（`state.selectedGroupKey = null` 後呼叫 `renderAll()`）不會碰到這個手動加的 class，導致 modal 關閉後卡片外匡一直亮著。修法：`closeEventDetailModal()` 在清空 `selectedGroupKey`／`openModalState` 之後、呼叫 `renderAll()` 之前，手動查找 `#dayEventsBody` 底下所有 `.loc-card-grid.selected` 並移除 `selected` class。動到 `js/events.js`。
    - **桌機（>991px）panel 展開推開版面時，跟 `.events-weekday-row` 留 20px 間距，並修正兩輪連帶跑版**：panel 本身固定貼齊視窗右緣、`max-width:360px`，一開始讓 `.events-page-body`／`#eventsWeekdayRow`／`.events-nav-group` 三個要推開的元素都用同一個 `margin-right: 380px`（= panel 寬度 360px + 20px 間距）。**跑版 1**：`.events-page-body` 自己另外有一份固定 20px 的 `padding-right`，跟 380px margin 疊加，變成比 `.events-weekday-row` 多縮進 20px（Gill 發現「看起來多了一段間距，比 events-weekday-row 更內縮」）；修法：`.events-page-body` 的 `margin-right` 扣掉這份既有 20px，改成 `360px`（360+20=380，總留白仍然一致）。**跑版 2**：`.events-nav-group` 所在的 `.events-filter-row` 本身 `padding-right` 是 16px（不是 20px），沿用 380px margin 會比月曆邊緣多縮 16px（Gill 發現「events-nav-group 應該要和月曆邊邊對齊」）；修法：`.events-nav-group` 的 `margin-right` 扣掉這 16px，改成 `364px`（364+16=380）。三個元素各自「既有右側留白」不同（`#eventsWeekdayRow` 沒有額外 padding，全靠自己的 margin；`.events-page-body` 有 20px padding-right；`.events-nav-group` 所在的 `.events-filter-row` 有 16px padding-right），margin-right 都要各自扣掉那份既有留白，三者加總後右邊界才會真的對齊在同一條線上（距視窗右緣 380px）。動到 `events.css`。

18. **日期 hover 外匡上邊框二次修正（第 16 點 z-index:3 的修法不夠）、`.events-bar` 左邊統一 12px 內縮、`.day-events-body` 左右 padding 調整、全站關閉鈕 icon 統一**（這次對話逐項討論確認）：
    - **日期 hover 外匡（`.events-week-hover-col`）上邊框，補了 `z-index:3` 之後實測還是被蓋住**：上一輪（第 16 點）判斷是 `.events-week-daynums` 的 `z-index:2` 蓋過 hover 外匡，補上 `z-index:3` 理論上應該解決；但 Gill 實測後回報「還是只有左右下三邊，上面那條線不見了」。根因不是單純的 z-index 排序問題——`.events-week-daynums` 同時是 `position: sticky` 元素，瀏覽器對「sticky 元素自己的合成層（compositing layer）」不一定完全照 z-index 排序繪製，實測還是會蓋掉跟它重疊的上邊框那一段。修法：不再只靠 hover-col 這個獨立疊層蓋過 daynums，改成**在 `.events-day-num-cell` 自己身上（daynums 這個 sticky 元素的內部子元素，跟它同一個合成層，不會有跨層蓋掉的問題）另外補畫上／左／右三邊**——新增 `.events-day-num-cell.col-hover { box-shadow: inset 0 1.5px 0 0 var(--fill-blue), inset 1.5px 0 0 0 var(--fill-blue), inset -1.5px 0 0 0 var(--fill-blue); }`，跟 hover-col 提供的左右下三邊疊在一起（左右邊會重複疊一次同色線，不影響視覺），湊出完整的四邊外框；`js/events.js` 的 `onDayCellHoverEnter()`/`onDayCellHoverLeave()` 同步在該日期格子上加／拿掉 `col-hover` class。動到 `events.css`／`js/events.js`。
    - **`.events-bar` 左邊統一 12px 內縮，不再只有從週一（第 1 欄）開始的橫幅有這個效果**：`.events-week-bars`（橫幅所在的 grid 容器）原本靠自己的 `padding-left:12px` 讓「第 1 欄」開始的橫幅左邊有 12px 內縮、對齊上面日期數字的 12px padding；但第 2～7 欄的橫幅左邊界就是 grid 內部分隔線本身，沒有額外空間，會直接貼齊那條直向格線——Gill 舉例「假設 bar 從 5 號開始，bar 的左側會直接貼齊邊匡」。跟 Gill 確認後選擇「所有 bar 一律 12px 內縮」的做法：`.events-week-bars` 拿掉 `padding-left`（改為 0），`.events-bar` 統一補上 `margin-left: 12px`，不管從第幾欄開始都有一致的 12px 內縮。**副作用（刻意接受）**：同一列裡兩個相鄰的橫幅（例如一個橫幅週三結束、下一個橫幅週四緊接著開始）之間的間距會從原本單純的 8px grid gap，變成 8px + 12px = 20px（下一個橫幅不分是不是緊接著前一個都會吃到這個 margin），換取「每顆橫幅起始邊都跟日期數字對齊」的一致性。動到 `events.css`。
    - **`.day-events-body` 左右 padding 從 12px 改成 16px**（≤991px；上方／下方維持原本的 8px／24px 不變）。動到 `events.css`。
    - **全站關閉鈕圖示統一改用 `close_20dp_000000_FILL1_wght400_GRAD0_opsz20`**：盤點後發現全站關閉鈕圖示不一致——`.grid-modal-close`（機台首頁）跟 `js/map.js` 產生的兩顆 `.popup-close-btn`（地圖詳情面板／cluster popup）原本是裸 unicode `✕` 字元；`.filter-sheet-close`（篩選／排序 bottom sheet，兩頁各兩顆）、`.changelog-close`（更新日誌，兩頁各一顆）、`.day-events-close`（當日活動 drawer）、events 頁的 `.grid-modal-close`（活動詳情 Modal）則是另一款 stroke 風格的 X SVG（`24x24`／`20x20` viewBox，兩條 `<path>` 畫叉叉），兩種風格混雜、也都不是專案 Icon 系統採用的 Material Symbols 圖示。統一做法：`js/utils.js` 新增共用常數 `CLOSE_BTN_ICON_SVG`（`fill="#000000"` 已比照專案慣例改成 `fill="currentColor"`），`js/map.js` 的兩顆 `.popup-close-btn` 改 import 使用；`app.html`／`events-app.html` 裡寫死的靜態關閉鈕（`.filter-sheet-close`×4、`.grid-modal-close`×2、`.changelog-close`×2、`.day-events-close`×1，共 9 顆）沒辦法 import JS 常數，直接把同一段 path 字串貼進 HTML（比照 `.events-nav-group` 月曆導航按鈕的既有慣例）。全站合計 11 顆關閉鈕，圖示現在完全一致。動到 `js/utils.js`／`js/map.js`／`app.html`／`events-app.html`。

19. **day-events-panel 卡片清單改成依寬度自動算欄數，只套用在 ≤991px（手機 bottom sheet）**（這次對話 Gill 先確認現況、再指定範圍）：
    - Gill 詢問「day-events-panel 的 list，card min width 是 330 對吧？排列方式，裝置寬度能放下幾欄就放幾欄，懂嗎？」——盤點後發現 `.events-card-list`（`events.css`）當時是寫死 `grid-template-columns: 1fr` 單欄，並沒有套用跟首頁 `.grid`／events 頁「總覽·列表」`.collage-list-grid`／地圖側邊欄 `.location-list` 同一套 `repeat(auto-fill, minmax(330px, 1fr))`（見上方第 11、12 點）邏輯，回報現況後跟 Gill 確認是否要一併套用
    - Gill 指示「只要 apply 到 小於等於 991 時就好了」——因為 panel 本身桌機（>991px）固定貼齊視窗右緣、`max-width:360px`（見上方 `.day-events-panel` 規則），就算套用同一套 `minmax(330px,1fr)`，360px 寬度也頂多還是塞得下一欄，改了沒有實質視覺效果；改法是只在既有的 `@media (max-width: 991px)` 區塊（手機 bottom sheet 版型）裡新增一條覆寫規則，桌機（base 規則）維持原本 `1fr` 單欄不變
    - 動到 `events.css`
20. **確認全站 loc card hover 樣式一致性（純確認，無程式碼變動）**：Gill 詢問「目前 loc card 的 hover 皆是一樣的樣式嗎？app 頁的列表、map side bar、event 頁的列表、calendar panel」——盤點結果：app 頁列表（`.loc-card-grid`）跟 map 側邊欄／mobile bottom sheet（`.loc-card`）在 `style.css` 裡的 `:hover` 規則逐字元相同（`border-color` + `translateY(-2px)` + `box-shadow: inset 0 0 0 1px var(--fill-blue), 0 8px 24px rgba(0,102,255,0.16)`）；events 頁「總覽·列表」跟 day-events-panel（見上一點，v35.2 已改沿用 `.loc-card-grid`）都直接沿用同一顆 class，`events.css` 只疊加 `.selected`／`.is-ended` 修飾、沒有另外覆寫 `:hover`。四處目前完全統一，沒有發現不一致，這次沒有動到任何檔案
21. **fix：events 頁「總覽·拼貼格狀」（`#eventsCollageGrid`）可以左右滑動的 bug**：Gill 回報「event 的總覽 grid 不應該能左右滑動」。根因是 `.events-collage-grid` 只設了 `overflow-y: auto`，沒有明確設 `overflow-x`——CSS 規格規定一軸是 `auto`／`scroll`、另一軸沒明確指定（預設 `visible`）時，瀏覽器會把那一軸隱性當成 `auto` 處理，不是 `visible`，跟專案裡 `.filter-panel` 之前踩過的同一個坑（見「列表 view-btn icon bug 與其他細部樣式調整」章節）。配合 `.collage-grid-col` 用 `flex:1 1 0` 分欄、卡片本身又有 `min-width:330px`（見上方第 11 點），欄數計算（`collageColumnCount()`）在 resize debounce（150ms）還沒跑完、或量測時機跟實際 render 有落差時，欄位會短暫跟卡片最小寬度對不上，這個隱性 `auto` 就會冒出橫向捲軸、可以左右滑動。修法：`.events-collage-grid` 明確補上 `overflow-x: hidden`，不管欄位計算有沒有完全跟上，都只會裁切、不會出現橫向滑動。動到 `events.css`

第 1、7 點動到 `js/events.js`／`events.css`；第 2～6 點動到 `style.css`／`events.css`／`events-app.html`／`js/sort-widget.js`／`js/events.js`；第 8 點動到 `js/utils.js`／`js/main.js`／`js/map.js`／`js/grid.js`／`js/events.js`／`style.css`／`events.css`／`app.html`／`events-app.html`；第 9 點動到 `events-app.html`／`js/scroll.js`；第 10 點動到 `js/utils.js`／`js/main.js`／`js/events-header.js`／`js/grid.js`／`js/map.js`／`js/events.js`；第 11 點動到 `style.css`／`events.css`／`js/events.js`；第 12 點動到 `style.css`／`js/map.js`；第 13 點動到 `js/map.js`／`style.css`；第 14 點動到 `style.css`／`events.css`／`js/events.js`；第 15 點動到 `js/utils.js`／`js/main.js`／`js/map.js`／`js/grid.js`；第 16 點動到 `events.css`／`js/events-scroll.js`；第 17 點動到 `events.css`／`js/events.js`；第 18 點動到 `events.css`／`js/events.js`／`js/utils.js`／`js/map.js`／`app.html`／`events-app.html`；第 19 點動到 `events.css`；第 20 點無程式碼變動；第 21 點動到 `events.css`。十六輪合計 13 個獨立檔案（`events.css`／`js/events.js`／`js/utils.js`／`js/main.js`／`js/map.js`／`js/grid.js`／`style.css`／`events-app.html`／`app.html` 多輪都有動到，這輪沒有新增檔案類型）。


**⚠️ v35.1 排序規則調整（本檔案這次更新時）尚未 commit／push**：拿掉「結束日：遠到近」（`end_date_desc`，v30.2 上線），新增「開始日：近到遠」（`start_date_asc`）；機台首頁跟活動行事曆頁兩邊都改了（原本第一輪只改首頁，`events.html` 是後來追加的第二輪）。動到 `js/utils.js`（新增 `getStartDate()`）、`js/grid.js`（`sortLocations()` 的 `end_date_asc`／`start_date_asc` 兩個分支改成各自分三組固定優先權排序）、`js/sort.js`（`SORT_OPTIONS` 拿掉 `end_date_desc`、加入 `start_date_asc`）、`js/events.js`（`EVENTS_SORT_OPTIONS` 同步調整，`compareGroupsBySort()`／`collageGroups()` 改成用「離今天天數差絕對值」跟「已開始/尚未開始分組」的邏輯，規則跟首頁相通但是各自獨立的程式碼），共 4 個檔案。完整規則見下方「排序系統」（首頁）跟「拼貼排序」（events）兩個章節的 v35.1 條目；順手校正了「拼貼排序」章節一條跟實際程式碼對不上的舊敘述（誤寫成只有三個距離排序選項），詳見該章節開頭的說明。
...
- **v35.2 修正：`to24Hour()`／`parseUpdateDate()` 跟上面這段「兩欄比較、組出顯示文字」的邏輯搬進 `utils.js`（新增 `buildLastUpdatedText(machineRaw, eventsRaw)`）**——這兩個函式原本是 `js/main.js`／`js/events-header.js` 各自複製一份逐字元相同的內容（`events-header.js` 不 import `main.js` 的理由見「為什麼是獨立頁面、獨立一批模組」，但這兩個函式本身零依賴、不摸 DOM，不受那個限制），改成兩邊都 import `utils.js` 這一份，之後要調整轉換格式或比較規則只要改一處。詳見「共用工具搬遷到 utils.js」章節
...
- **預設狀態（`sheetLoc` 為 null）**：顯示可捲動的地點卡片列表（`buildSidebarListCardHtml` + `bindSidebarListCardEvents`，跟桌機、grid 共用同一套卡片渲染），取代 v20 版的提示文字
  - **v35.2 新增：卡片清單依寬度自動算欄數**（`.location-list.card-grid`，見「Desktop 地圖側邊欄」章節同一條筆記）；渲染詳情內容時移除 `card-grid`，避免 `buildDetailContentHtml()` 的多個 top-level 區塊被 grid 拆欄
  - **v35.2 新增：詳情內容的關閉鈕改為 sticky 固定**——原本跟 badge 列包在同一個 flex row 裡，整段都是 `#locationList` 捲動內容的一部分，內容長時關閉鈕會被捲出視野；改成 badge 列（`.detail-badge-row`）維持原本跟著內容捲動，關閉鈕包一層 `.detail-close-btn-wrap`（`position:sticky; top:0; height:0;`，`height:0` 不佔版面、不把 badge 往下推），按鈕本身用 `.popup-close-btn.detail-close-btn`（`position:absolute; top:0; right:16px`，兩個 class 疊加蓋過基礎 `.popup-close-btn` 的 `top:4px;right:4px`）相對 wrapper 定位到右上角；外觀不加圓底/陰影，比照 `.grid-modal-close` 的純圖示樣式
...
- **v35.2 新增：`.location-list` 卡片清單改依寬度自動算欄數**（`display:grid; grid-template-columns: repeat(auto-fill, minmax(330px, 1fr));`），桌機固定 400px 寬其實還是只會排出 1 欄，手機 bottom sheet 橫向夠寬時才會變多欄；因為 `#locationList` 卡片清單跟機台詳情共用同一個容器，grid 規則收進 `.card-grid` 修飾 class，只在渲染卡片清單時加上、渲染詳情時移除，避免詳情內容（`buildDetailContentHtml()` 產出的好幾個 top-level 區塊）被一起拆欄。詳見「Mobile Map Bottom Sheet」章節同一條 v35.2 筆記
- **v35.2 新增：詳情面板關閉鈕改為 sticky 固定**，不隨內容捲動——見下方「Mobile Map Bottom Sheet」章節同一條 v35.2 筆記（桌機／手機共用同一份 `buildDetailContentHtml()`，改動對兩邊同時生效）
...
### 排序系統（Sort，v22 新增，v35.1 拿掉 end_date_desc、新增 start_date_asc）
...
**v35 追加**：`app.html` 的 `#btnGrid`／`#btnMap` 這兩顆 view-toggle 按鈕的 `onclick`，從單純 `setView('grid')`/`setView('map')` 補上 `window.syncSearchUrl && window.syncSearchUrl();`；`js/main.js` 對應新增 `window.syncSearchUrl = syncSearchUrl;` 讓 inline `onclick` 能呼叫到（比照 `window.shareLocation` 既有的跨檔案呼叫慣例；`window.filterByCharacter` 這個對照對象本身 v35.2 起已經拿掉，改用 data-* + 事件委派，見「作品標籤點擊快速篩選」章節 v35.2 補記，這裡沿用原文不追溯修改，只補充現況）。純粹手動切換 grid／map 版面（沒有搜尋/篩選變動）過去不會同步網址，現在會——跟 `events.html` 這次同一輪加上的「總覽／月曆分頁籤也同步網址」是對稱的修正（見下方「搜尋結果分享連結：`eventsSyncSearchUrl()`」的 v35 段落）。
...
**`filterByCharacter(character, machineId, source)`（`main.js`，v35.2 前掛在 `window` 讓 `map.js` 動態產生的 HTML 也能呼叫，比照 `trackGmapsClick`／`shareLocation` 既有的跨檔案呼叫慣例；v35.2 起改法見下方補記）**：
...
- ~~作品名稱塞進 inline `onclick` 屬性前先跳脫單引號，避免作品名稱本身含英文撇號（例如羅馬拼音）時把 `onclick` 字串弄斷~~——**v35.2 起這段跳脫邏輯已隨 inline onclick 一起拿掉**，見下方補記
...
**v35.2 補記：`filterByCharacter`／`trackGmapsClick` 改用 `data-*` 屬性＋事件委派，拿掉 `window` 掛載**——這次對話討論「main.js／map.js／events.js 哪些綁定方式能不能共用」時，順著這個問題找到的兩個具體改動點。原本 `main.js`（grid modal）、`map.js`（地圖詳情面板，桌機側邊欄／mobile bottom sheet 共用同一個 `buildDetailContentHtml()`）、`grid.js`（列表卡片的 Google Maps 連結）都各自組出 inline `onclick="filterByCharacter(...)"`／`onclick="trackGmapsClick(...)"` 這種呼叫字串，改成純資料屬性——作品按鈕加 `data-character`／`data-machine-id`／`data-source`，gmaps 連結加 `data-gmaps-track`／`data-machine-id`／`data-source`；`main.js` 既有處理 lightbox／grid carousel 的那個 document click 委派監聽器（見「Lightbox（grid）」段落）新增兩段判斷，分別比對 `.popup-character-link[data-character]`／`[data-gmaps-track]` 呼叫對應函式（gmaps 連結本身是原生 `<a target="_blank">` 導頁，這裡只送 GA，不 `preventDefault`）；`filterByCharacter`／`trackGmapsClick` 改成 `export`，兩行 `window.X = X` 掛載拿掉。**副作用**：手動跳脫單引號那段邏輯不再需要——資料屬性不是拼接 JS 程式碼字串，作品名稱含撇號、雙引號等特殊字元都不會把綁定弄斷（相較 inline onclick 原本只跳脫了單引號，其他特殊字元其實仍有風險，這次一併排除）。`map.js`／`grid.js` 因為不再需要 inline onclick，這兩個函式也不用額外 `import` 進去，委派監聽器統一掛在 `main.js`、涵蓋整個 `document`，跨模組產生的 HTML 都接得到。**`shareLocation`／`trackEventTitleClick`／`closeDetailPanel` 等其餘 inline onclick＋window 掛載的函式這次刻意沒有一起改**，屬於範圍最小、風險可控的局部改動，全站還有多處同樣模式，之後如果要繼續收斂建議整批規劃、一次做完，不要分散著改。動到 `js/utils.js`（新增 `createLightbox`，跟這次改動同一輪）／`js/main.js`／`js/map.js`／`js/grid.js`，共 4 個檔案，詳見上方「共用工具搬遷到 utils.js」章節。
...
- **v35.2 小修**：`js/main.js` 的 `openGridModal()` 原本在同一段模板字串裡 `getEndingBadge(loc.limited)` 被呼叫兩次（判斷式一次、組內容字串又一次），改成只算一次存進 `endingBadge` 變數，跟 `grid.js`／`map.js` 既有寫法一致
...
- **抽卡機／相卡機 type-badge 圖示搬進 `utils.js`（v35.2）**：`MACHINE_TYPE_BADGE_ICON`（圖示 SVG）跟新增的 `machineTypeClass(type)`（回傳 `gacha`/`photocard`）原本是 `grid.js`／`main.js`／`map.js`（3 處）各自複製一份逐字元相同的 inline SVG 字串，`events.js` 也是獨立宣告一份同名常數，改成四個檔案都 import `utils.js` 這一份；順手補上圖示來源註解，`抽卡機` 對應 Material Symbols `playing_cards`、`相卡機` 對應 `photo_camera`（皆 FILL1），跟 `svg icon/` 資料夾裡實際下載的原始檔名對齊。`map.js` 裡地圖 marker 專用的 `MARKER_ICON_SVG`（`L.divIcon` 用，屬性跟一般 type-badge 不同，見「地圖 Marker 設計」）刻意不納入共用，只有顏色 class 判斷（`machineTypeClass`）一起統一。詳見「共用工具搬遷到 utils.js」章節
- **跨檔案重複的 inline SVG 抽成 `utils.js` 共用常數（v35.2）**：`BTN_EXPAND_ICON_SVG`（`.btn-expand`，`expand_content` FILL1，16px）、`SHARE_BTN_ICON_SVG`（`.popup-share-btn`，`share` FILL0，16px）、`CAROUSEL_CHEVRON_ICON_SVG`（`.carousel-btn`／`.related-machines-nav-btn`，`chevron_backward` FILL0，14px）。這幾顆圖示原本都是逐字元重複貼在多個檔案裡的 inline SVG 字串，改成 import 同一份共用常數，理由跟既有的 `MACHINE_TYPE_BADGE_ICON` 一致——純字串、零依賴，不受「`events.js` 不能 import `main.js` 那條鏈」的限制。方向性箭頭（carousel prev/next、相關機台卡片列、月曆上/下個月）統一只準備一顆朝左的 `chevron_backward`，反方向靠 CSS `transform: scaleX(-1)` 鏡射，不重複準備 forward icon；三處鏡射的是哪一個方向不一定相同（carousel 是鏡射 next、相關機台跟月曆導航也是鏡射 next，只是各自的預設朝向剛好一致，並非巧合以外的規則），改動前先看清楚原本 icon 的朝向再決定鏡射哪一顆。`events-app.html` 的 `.events-nav-group`（月曆上/下個月）因為是寫死在 HTML 裡、無法 import JS 常數，是把同一段 path 字串直接貼進 HTML，不是共用常數
...
| `sort_change`（v22，v30.2 曾新增 `end_date_desc`，v35.1 移除、新增 `start_date_asc`） | 選擇排序方式並實際套用（距離排序需等定位成功才觸發，選了但定位失敗不算） | `sort_key`(end_date_asc/start_date_asc/distance_asc/distance_desc), `device` |
...
- 但有 **3 個既有維度的「說明」欄位文字沒有跟著新值更新**，內容還停在舊版本，需要去 GA4 後台手動編輯：
  - **排序方式**（`sort_key`）：目前只寫 `end_date_asc/distance_asc/distance_desc`，缺 v30.2 新增、v35.1 已移除的 `end_date_desc`，也缺 v35.1 新增的 `start_date_asc`——回頭補charts說明文字時直接寫最終值（`end_date_asc/start_date_asc/distance_asc/distance_desc`），不用把已經移除的 `end_date_desc` 也列進去
...
- **v35.2 修正：`events-app.html` 補齊 `app.html` 原本就有的固定高度寫法，順便修掉訪客數在活動頁永遠顯示不出來的 bug**——`app.html` 的 `#visitorBanner` 本來就是 inline `visibility:hidden`；`events-app.html` 複製貼上時留成 `display:none`。`js/visitor.js` 抓到人數成功時只會把 `visibility` 切成 `visible`，從沒動過 `display`：行內 `display:none` 優先度比 class 的 `display:flex` 高、也不受 `visibility` 影響，`events-app.html` 這顆 banner 因此不管 API 有沒有抓到人數都一直是 `display:none`，等於永遠顯示不出來（只有失敗分支 `style.display='none'` 有效果）。修法是把 `events-app.html` 也改成 `visibility:hidden`：CSS 預設的 `display:flex` 讓它從一開始就佔著版面空間，只是視覺上先隱形，不會在人數抓取成功那一刻才讓 `#topBar`/`#eventsTopBar` 高度跳動；`js/visitor.js` 邏輯本身不用改。這個改動連帶讓「#topBar 滑動隱藏」機制不再需要 `ResizeObserver` 持續監看，見下方該章節條目
...
- **`#gridView` 的 `padding-top`**：因為 `#topBar` 變成 `fixed`（脫離文件流），`#gridView` 要保留對應高度避免第一批卡片被蓋住，寫成 `calc(12px + var(--top-bar-height, 0px))`；`--top-bar-height` 這個 CSS variable 由 `js/scroll.js` 動態量測 `#topBar.getBoundingClientRect().height` 寫入，**不是寫死的數字**
  - **量測時機的坑（v35.2 修正）**：訪客計數 banner 原本是非同步出現的（`visitor.js` 抓到人數後才把 `display:none` 打開），如果只在 `resize` 時重新量測，banner 突然跳出來的那一瞬間高度會沒跟上、內容被蓋住一小段；當時改用 `ResizeObserver` 直接盯 `#topBar` 本身的尺寸變化解決。v35.2 起 banner 改成一開始就用 `visibility:hidden` 保留固定高度（見「訪客計數 Banner」章節），不再有「內容非同步出現、高度中途變動」的情況，`ResizeObserver` 因此拿掉，`updateTopBarHeight()` 改成只在 `initTopBarScroll()` 初始化跟 `window resize`（手機/桌機斷點切換）時重算；KADO 不需要處理裝置旋轉這種情境，這裡的 resize 監聽單純是給斷點切換用的。如果之後又出現會動態改變 `#topBar` 高度的內容，再視情況加回 `ResizeObserver`
...
**v35.2 新增兩組**（跟上面這批「events.js 也要用」的理由不同：這兩組是 `js/main.js` 跟 `js/events-header.js` 之間的重複，不是首頁 chain 跟 events.js 之間的重複，但一樣是零依賴的純邏輯，適合放進 `utils.js` 共用，不受「events.js 不能 import main.js」那條限制）：
- `to24Hour()`／`parseUpdateDate()`／`buildLastUpdatedText(machineRaw, eventsRaw)`（原各自在 `main.js`／`events-header.js`）——「最後更新」時間格式轉換跟兩欄比較、組出顯示文字的邏輯，詳見「最後更新元素」章節 v35.2 條目
- `MACHINE_TYPE_BADGE_ICON`／`machineTypeClass(type)`（原各自在 `grid.js`／`main.js`／`map.js`／`events.js`）——抽卡機/相卡機徽章圖示跟顏色 class 判斷，`map.js` 裡地圖 marker 專用的 `MARKER_ICON_SVG` 是屬性不同的另一組圖示，刻意不共用，詳見「Icon 系統」章節 v35.2 條目

**v35.2 再新增一組**（跟上面兩組理由相同，`main.js`／`events.js` 之間的重複，零依賴純 DOM 操作）：
- `createLightbox(overlayId, imgId)`（原各自在 `main.js`／`events.js`，回傳 `{ open, close }`）——Lightbox「設定 `img.src`、toggle overlay 的 `show` class」這段核心開關邏輯，兩邊各自呼叫一次拿獨立實例，只共用這一小段，呼叫端的 GA 事件與綁定方式（機台版 inline onclick＋`window` 掛載／events 版 `addEventListener`）刻意不一起搬，詳見「圖片放大 Lightbox」章節的 v35.2 補記
...
**兩個跨頁連結的 `target` 刻意不同（v35.4 起改成只有桌機才不同）**：`machineTitleHtml()` 產生的機台標題連結（機台→活動）一律沒有 `target="_blank"`，同頁跳轉。`relatedMachinesHtml()` 產生的相關機台卡片連結（活動→機台）原本不分裝置一律 `target="_blank" rel="noopener"`，另開分頁；v35.4 改成只有桌機（`isMobileFilterLayout()` 判斷 991px 以上）才另開分頁，手機一律同頁跳轉（Gill 回報：手機切換機台與活動時不該另開分頁，體驗上比較像不小心跳出去，活動 Modal 也不會留在背景給人「切回來」的直覺）。桌機維持另開分頁：使用者可能想依序點好幾台相關機台比較，或者看完某台機台後還想回來繼續看同一個活動的其他資訊（分享、地圖等），另開分頁能保留原本開著的活動 Modal 狀態，不用重新點一次進來。桌機這個理由是事後從程式碼行為反推、不是當初 commit 訊息或既有文件寫明的決策依據，之後如果要再調整記得同步這裡。
...
- **DOM 位置**：靜態 HTML 裡 `#eventsTimeFilterPill`／`#eventsMachineFilterPill` 這兩顆按鈕寫在 `events.html`（原本外面包了一層 `.quick-filter-group` wrapper，後來拿掉，改成各自獨立的按鈕，讓它們能吃到 `.filter-scroll` 的 `gap:12px`，跟其他 pill 間距一致），由 JS 的 `positionQuickFilterGroup()` 在初始化跟每次 `filterWidget.render()` 之後把這兩顆各自搬進 `#eventsFilterBar` 裡的 `.filter-scroll`（跟類型／作品／縣市 pill 同一個容器），夾在篩選 pill 跟排序按鈕（`.sort-group`）中間。**必須搬進 `.filter-scroll` 內、而不是當成它的 sibling**：`.filter-scroll` 才有 `overflow-x: auto`（≤991px 生效），搬到外面會導致這兩顆快速篩選 pill 在手機版無法跟類型／作品／縣市三顆 pill 一起左右滑動——這是實作過程中發現並修正的問題。`filterWidget.render()` 會整個清空重畫 `#eventsFilterBar`，所以每次重畫後都要再呼叫一次 `positionQuickFilterGroup()` 把這兩顆重新搬進去。
...
### 拼貼格狀排版：JS 輪流分欄取代 CSS multi-column（v32，v35.2 欄數邏輯改為動態量測）
原本用純 CSS multi-column（`column-count`）排版，缺點是閱讀順序會變成「先填滿第一欄、再填第二欄」，不是使用者預期的「由左到右、由上到下」。改成 JS 依序把卡片「輪流」分配到各欄（卡片 0→欄0、1→欄1、2→欄2...、N→欄0...），欄位本身仍用 flex 直排、圖片原生比例決定卡片高度，維持無縫貼齊的瀑布流視覺；差別只在「哪張卡放進哪一欄」改成依序輪流，不是照欄位目前高度最短的去放，讀者由左到右掃過去看到的就是資料原本的排序（跟月曆一致：快結束的在前）。代價是同一「列」的卡片底部不會剛好切齊，但比讀取順序錯亂更能接受。

**v35.2**：`.collage-card` 補上 `min-width: 330px`（見「機台首頁列表／events 頁...卡片格線 min-width 統一調整為 330px」條目）之後，原本「欄數要跟 `events.css` 的 `.events-collage-grid` 對應斷點（1100px／720px／420px）保持一致」這套純靠螢幕寬度分段的固定斷點邏輯，會在欄數跟卡片最小寬度對不上時把卡片擠爆容器造成橫向溢出。`collageColumnCount()` 改成量測 `#eventsCollageGrid` 的 `clientWidth`，用 `Math.floor(width / COLLAGE_CARD_MIN_WIDTH)` 反推能塞下幾欄（`COLLAGE_CARD_MIN_WIDTH = 330`，上限 `COLLAGE_MAX_COLS = 4`、下限 1 欄），取代原本 420/720/1100 三個寫死的數字；resize debounce（150ms）判斷欄數是否變動、要不要重新分配重繪的邏輯不變，只是欄數本身的算法換了。之後如果又要調整卡片最小寬度，只要改 `events.css` 的 `.collage-card` 跟 `js/events.js` 的 `COLLAGE_CARD_MIN_WIDTH` 這兩處，不用再回頭找斷點數字。

### 拼貼排序（v32，v35.1 調整選項與方向邏輯）
拼貼排序沿用共用的 `sort-widget.js`。**⚠️ 本節標題原本記錄「只有三個選項，沒有距離：遠到近」，跟實際程式碼（`EVENTS_SORT_OPTIONS` 一直都有 `distance_desc`）不符**——回頭查不出這份文件跟程式碼是哪個時間點開始對不上，v35.1 這次順手校正成跟現況一致，之後如果又要調整距離排序方向數量，記得這裡也要同步改。

`EVENTS_SORT_OPTIONS`（v35.1 起）：`end_date_asc`（default）、`start_date_asc`、`distance_asc`、`distance_desc`，跟首頁 `js/sort.js` 的 `SORT_OPTIONS` 選項組合完全一致，但這是兩份獨立宣告、獨立維護的陣列（`events.js` 刻意不 import `js/sort.js`，理由見上方「為什麼是獨立頁面、獨立一批模組」），改一邊不會自動同步到另一邊。v35.1 比照首頁排序調整（見「排序系統」章節 v35.1 條目）拿掉 `end_date_desc`、新增 `start_date_asc`，理由跟規則兩邊相通，不重複寫一次。

排序結果不是直接決定畫面順序，是餵給總覽的「還沒結束」／「已結束」分組（`collageGroups()`，依 `group.end >= 今天` 分組）各自內部排序，分組本身固定不受排序影響——這個分組結構剛好跟首頁 v35.1 新增的「已過期排最後」概念殊途同歸，只是首頁是排序邏輯裡內建的規則，這裡是本來就存在、獨立於排序鍵之外的分組。`compareGroupsBySort(a, b, sortKey, userCoords, today)`：
...
- `start_date_asc`（v35.1 新增）：已開始（`start <= 今天`）優先權高於尚未開始（`start > 今天`），已開始組內依開始日新到舊（離今天最近排最前），尚未開始組內依開始日由近到遠（越快開始排最前）；`ended` 分組裡的 group 必定已經開始過，天然全部落在「已開始」這一支，不會意外混進「尚未開始」
...
**v35.2 補記：核心開關邏輯搬進 `utils.js` 共用，綁定方式仍刻意保持不同**——`openLightbox`/`closeLightbox`（機台版）跟 `openEventLightbox`/`closeEventLightbox`（這裡）原本各自把「設定 `img.src`、toggle overlay 的 `show` class」這兩行完全重複的邏輯各寫一份，抽成 `utils.js` 的 `createLightbox(overlayId, imgId)`（回傳 `{ open, close }`），兩邊各自呼叫一次拿獨立實例，只共用這一小段純 DOM 操作。**上一段講的綁定方式差異（機台走 inline onclick＋`window` 掛載，這裡走 `addEventListener`）完全沒變**——這兩件事是分開的層次：核心開關邏輯可以共用，但「誰來呼叫、怎麼呼叫」還是各自頁面既有慣例決定，沒有理由因為底層共用了就強迫綁定方式也要統一，詳見「共用工具搬遷到 utils.js」章節的 v35.2 條目。
...
`.events-filter-row .filter-bar { background:none; padding:0; }` 這條規則原本寫的是 `.events-calendar-col .filter-bar`，是 v40 重構（搜尋列／filter-row／星期列從 `.events-calendar-col` 搬進 `#eventsTopBar`／`.events-topbar-controls`）之前的舊選擇器。重構後 `#eventsFilterBar` 已經不在 `.events-calendar-col` 底下了，這條規則變成完全比對不到任何東西的死規則——安靜地失效、不報錯、肉眼也看不出差異，直到實測才發現：`filter-bar` 自己的左右 20px padding 疊加在外層 `.events-topbar-controls` 的 20px padding 上面，手機版第一個 pill 距離視窗左緣變成 40px（20+20），不是預期的 20px。改成 `.events-filter-row .filter-bar`，比對到現在實際包著 `#eventsFilterBar` 的容器，覆寫才真的生效。**教訓**：DOM 結構搬遷（v40 這類把區塊移進新 wrapper 的重構）之後，記得檢查原本綁在舊祖先選擇器上的規則有沒有變成死規則——CSS 對比對不到的選擇器不會有任何警告。（後續更新：`.events-topbar-controls` 這層 wrapper 後來整個拿掉了，`.toolbar`／`.events-filter-row`／`.events-weekday-row` 現在直接是 `#eventsTopBar` 的子元素，各自管自己的左右 padding／margin，比照 app 頁 `#topBar` 底下 `.toolbar`／`.filter-bar` 平行、各自獨立的結構，不再需要額外的分組 wrapper。）
...
每一週橫幅的擺放分兩步：先算每個 group 在這一週實際涵蓋的欄位範圍（`colStart`／`colSpan`，只算這一週裡真的有顯示日期的欄，不延伸到留白格），再排序決定處理順序，最後用 `rowLastCol` 陣列（記錄每個 row 目前佔用到的最後一欄）做簡單的區間排程：找第一個「目前佔用範圍在這個 bar 開始欄位之前」的 row 放進去，找不到就開新 row。

**v35.2 改成「剩餘天數優先」**：排序 key 改成 `daysUntilEnd(group, today)`（活動距離結束還剩幾天，已結束回傳 `Infinity` 排最後）由小到大排，同樣剩餘天數才退回原本那組橫向排列規則（開始欄位小的優先、同開始欄位時橫跨天數多的優先）當 tie-break。這是 Gill 明確要的「行動導向」排序——快結束的活動要優先被看到，提醒使用者「這個快沒了要趕快去」；代價是這個排序不再照開始欄位排，餵給下面貪婪打包演算法時，可能沒辦法拿到列數最少的最佳排法（一個橫跨多天的長活動可能因為剩餘天數比較多，被一個快結束的短活動擠到後面、掉到下一列），是刻意接受的取捨。
